...
/**
 * Scenario to get property content in an additional language.
 */
public class GetPropertyContentInAdditionalLanguageScenario implements RapidScenario {
  private static final Logger logger =
      LoggerFactory.getLogger(GetPropertyContentInAdditionalLanguageScenario.class);
  private ContentService contentService = new ContentService();
  private RapidPartnerSalesProfile rapidPartnerSalesProfile;

  /**
   * Sets the profile for the scenario.
   *
  ...
  /**
   * Runs the scenario to get property content in an additional language.
   */
  @Override
  public void run() {
        ...
        "en-US", this.rapidPartnerSalesProfile).thenApply(propertyContentMap -> {
          if (propertyContentMap.getData().isEmpty()) {
            throw new IllegalStateException(
              "No property content found for property id:" + Constants.TEST_PROPERTY_ID);
          }
          ...
          Map<String, GetPropertyContentOperationLink> selectedPropertyLinks =
              propertyContentMap.getData().entrySet().stream().findFirst().get().getValue()
                .getAddress().getLocalized().getLinks();

          if (selectedPropertyLinks.isEmpty()) {
            throw new IllegalStateException(
              "No links found to property content in other languages."
            );
          }

          selectedPropertyLinks.forEach((language, link) -> logger.info(
              "Language: [{}], Link: [{}]",
              language,
              link.getHref()
          ));
          ...
          PropertyContent propertyContent =
              propertyContentMap.entrySet().iterator().next().getValue();

          return contentService.asyncGetPropertyContentInAdditionalLanguage(
            propertyContent,
