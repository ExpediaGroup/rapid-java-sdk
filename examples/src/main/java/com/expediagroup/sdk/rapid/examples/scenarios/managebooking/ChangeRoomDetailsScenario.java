...
/**
 * Scenario to change room details for a booking.
 */
public class ChangeRoomDetailsScenario implements RapidScenario {
  ...
  /**
   * Runs the scenario to change room details for a booking.
   */
  @Override
  public void run() {
    ...
    logger.info("Change room details for room [id:{}] in itinerary...",
        itinerary.getRooms().get(0).getId());
    Response<Nothing> response = bookService.changeRoomDetails(itinerary, 0);
    logger.info("Change room details for room [id:{}] response status: [{}]",
        itinerary.getRooms().get(0).getId(), response.getStatusCode());
